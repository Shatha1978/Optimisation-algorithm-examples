...
def isValid(algorithm, mode, selection):
    ...
    if algorithm == "PSO" and mode == "" and selection == "":
        return True;
    elif algorithm == "RCGA" and mode == "generational":
        if selection == "roulette" or selection == "ranking" or selection == "tournament":
            return True;
    elif (algorithm == "FA" or algorithm == "FA-no_mitosis") and (mode == "generational" or mode == "steady_state"):
...
def getConfigurationPath(problem_size, algorithm, mode, selection):
    ...
    if algorithm == "FA" or algorithm == "FA-no_mitosis":
        path += "-" + mode;

    if algorithm == "RCGA" or algorithm == "FA" or algorithm == "FA-no_mitosis":
        path += "-" + selection;
...
for problem_size in problem_size_set:
    ...
    fig.savefig("global_fitness_evolution-pb_size-" + str(problem_size) + ".pdf", bbox_inches='tight')
...
for algorithm in algorithm_set:
    ...
    if algorithm == "PSO" or algorithm == "RCGA":
        ...
        for mode in ["generational", "steady_state"]:
            if (algorithm == "FA" or algorithm == "FA-no_mitosis") and mode == "generational":
...
fig.savefig("mean_global_fitness_lamps-log-x.pdf", bbox_inches='tight')
...
fig.savefig("global_fitness_lamps-PSO-RCGA.pdf", bbox_inches='tight')
...
fig.savefig("global_fitness_lamps-FA.pdf", bbox_inches='tight')
...
fig.savefig("global_fitness_lamps.pdf", bbox_inches='tight')
